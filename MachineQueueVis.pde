...
void initHUDs(){
  ...
  jobBox[2] = "Job Priority: " + smallJobs.get(highlighter1).getJobPrio();
  jobBox[3] = "Job Name: " + smallJobs.get(highlighter1).getJobName();
  jobBox[4] = "Job Owner: " + smallJobs.get(highlighter1).getJobOwner();
  jobBox[5] = "Job Start Time: " + smallJobs.get(highlighter1).getStartTime();
  jobBox[6] = "Queue Name: " + smallJobs.get(highlighter1).getQueueName(); 
  jobBox[7] = "Slot Count: " + smallJobs.get(highlighter1).getSlots();
...
void parseFile() {
  ...
  for (int i=0; i < json.size(); i++ ) {    
    ...
    if(job.getJSONArray("State").toString().equals("[\"ipf:running\"]")) { // only process running jobs
      ...
      float prio = job.getJSONObject("Extension").getFloat("Priority");
      ...
      if(slotNum < smallJobsUpperBound) smallJobs.add(new Job(num, prio, name, owner, startTime, queue, slotNum));
      else if(slotNum > (smallJobsUpperBound-1) && slotNum < mediumJobsUpperBound) mediumJobs.add(new Job(num, prio, name, owner, startTime, queue, slotNum));
      else largeJobs.add(new Job(num, prio, name, owner, startTime, queue, slotNum));
    }
  }
}
...
void updateHUD(Helix helix, ArrayList<Job> jobs, int jobIndex, String helixType){
  ...
  jobBox[2] = "Job Priority: " + jobs.get(jobIndex).getJobPrio();
  jobBox[3] = "Job Name: " + jobs.get(jobIndex).getJobName();
  jobBox[4] = "Job Owner: " + jobs.get(jobIndex).getJobOwner();
  jobBox[5] = "Job Start Time: " + jobs.get(jobIndex).getStartTime();
  jobBox[6] = "Queue Name: " + jobs.get(jobIndex).getQueueName(); 
  jobBox[7] = "Slot Count: " + jobs.get(jobIndex).getSlots(); 
